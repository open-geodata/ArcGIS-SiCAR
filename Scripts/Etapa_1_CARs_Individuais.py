...
# -------------------------------------------------------------------------------------------------------
# Variável de Input
Input = r'Y:\SIG_IC_09-16\Dados Brutos\CARs\1'
...
# Cria a pasta 'Dados Brutos', caso não exista.
    ...
    directorys = ['Dados Brutos', 'Geodata',
                  'Documentos', 'Geodata//shp', 'Geodata//kml']
...
# Cria Geodatabase
    try:
        arcpy.CreatePersonalGDB_management(os.path.join(
            Input, folder, 'Geodata'), 'Geo_SiCAR.mdb')
    except:
...
# Renomeia PDF com a variável "Número do CAR criada anteriormente".
    ...
    for file in glob.glob(pdflist):
        ...
        os.rename(file, os.path.join(
            pdfpath + "//Extrato CAR " + numberfolder + fileext))
...
# Importa KMLs: Transforma KML que vira um geodatabase
    ...
    for file in glob.glob(kmllist):
        try:
            ...
            gdb = str.replace(file, '.kml', '.gdb') + \
                '//Placemarks//Multipatches'
            out = os.path.join(Input, folder, 'Geodata', "Geo_SiCAR.mdb", str.replace(
                os.path.basename(file), '.kml', ''))
...
# Renomeia e Move demais arquivos (shapefiles)
    ...
    for file in otherfiles:
        ...
        if parte1 == 'Nascente' or parte1 == 'AppsUnificadas':
            # ou AppsUnificadas DELETAR
            print 'Diferenciar Nascentes/AppsUnificadas de Pontos daquelas representadas por Polígons'
            newname = os.path.join(
                otherspath, parte1 + '_' + numberfolder + '_' + filename_zero + fileext)
            try:
                os.rename(file, newname)
                shutil.move(newname, os.path.join(
                    Input, folder, 'Geodata//shp'))
            except OSError:
                pass

        else:
            newname = os.path.join(
                otherspath, parte1 + '_' + numberfolder + fileext)
            try:
                os.rename(file, newname)
                shutil.move(newname, os.path.join(
                    Input, folder, 'Geodata//shp'))
    ...
    for dirpath, dirnames, filenames in walk:
        for filename in filenames:
            ...
            arcpy.CalculateField_management(
                filefullpath, 'Nome_Arquivo', "'" + filename + "'", 'PYTHON_9.3')

            arcpy.AddField_management(filefullpath, 'Layer', 'TEXT')
            arcpy.CalculateField_management(
                filefullpath, 'Layer', '!Nome_Arquivo!.split("_")[0]', 'PYTHON_9.3')

            arcpy.AddField_management(filefullpath, 'N_CAR', 'TEXT')
            arcpy.CalculateField_management(
                filefullpath, 'N_CAR', "'" + numberfolder + "'", 'PYTHON_9.3')

            arcpy.AddField_management(filefullpath, 'URL', 'TEXT')
            arcpy.CalculateField_management(
                filefullpath, 'URL', "'" + "..\Documentos\Extrato CAR " + numberfolder + ".pdf" + "'", 'PYTHON_9.3')
...
# Apenas para Polígonos
    ...
    for dirpath, dirnames, filenames in walk:
        for filename in filenames:
            ...
            arcpy.CalculateField_management(
                filefullpath, 'Area_ha', '!shape.geodesicArea@hectares!', 'PYTHON_9.3')
